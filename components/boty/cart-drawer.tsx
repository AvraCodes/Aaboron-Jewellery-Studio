              ...
              {items.map((item) => (
                <div key={`${item.id}-${item.variant ?? ''}`} className="flex gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="font-serif text-base text-foreground mb-0.5 font-semibold">{item.name}</h3>
                    {item.variant && (
        ...
        {items.length > 0 && (
          <DrawerFooter className="border-t border-border/50 p-6 gap-4">
            ...
            <DrawerClose asChild>
              <Link
                href="/checkout"
                className="w-full bg-primary text-primary-foreground py-4 rounded-full font-medium hover:bg-primary/90 boty-transition text-center text-sm tracking-wide"
              >
                Proceed to Checkout
              </Link>
            </DrawerClose>
            ...
            <DrawerClose asChild>
              <button
                type="button"
                className="w-full border border-border text-foreground py-4 rounded-full font-medium hover:bg-muted boty-transition text-sm"
              >
                Continue Shopping
              </button>
            </DrawerClose>
